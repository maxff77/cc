...
- `kind='cc'` → **Filtrada**: extracted `CC:` data, **deduplicated for the tenant's whole life by a partial unique index** `uq_responses_session_cc(capture_session_id, text) WHERE kind='cc'` (enforced in the DB, not code; text truncated to 600 chars for the btree limit). Since each tenant now has exactly one perpetual `capture_session`, this index — once per-rotating-session — now dedups a CC value tenant-lifetime. Limpiar does NOT touch this dedup `SELECT`.
...
- **Capture/response semantics (legacy parity).** Status from the latest ✅/❌ revision; only ✅/❌ persist (a pure ⏳ writes nothing). CC dedup is tenant-lifetime and DB-enforced; don't move it into code.
