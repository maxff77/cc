...
# Buscadores para entrar a Amazon como humano (clic real -> Referer del buscador). DDG html lite:
# markup estable (a.result__a), liviano, sin CAPTCHA agresivo con IPs de proxy (Google sí lo tira).
SEARCH_ENGINES = [
    {"name": "DuckDuckGo", "base": "https://duckduckgo.com/",
...
# Palabras "humanas" para contraseñas plausibles (animal/color/naturaleza/comida/positivo, estilo
# RockYou real). NO se reusan FIRST/LAST: ligar la pass al nombre de la cuenta sería un tell de
# correlación. Todas ASCII lowercase -> tecleo seguro en el form.
PW_WORDS = (
    # animales
    "dragon", "tiger", "monkey", "panda", "falcon", "eagle", "shark", "wolf", "lion", "cobra",
    "raven", "otter", "lynx", "bison", "moose", "gecko", "viper", "mantis", "badger", "walrus",
    "ferret", "jaguar", "puma", "heron", "sparrow", "hawk", "owl", "fox", "bear", "dolphin",
    "octopus", "penguin", "koala", "rhino", "camel",
    # colores
    "purple", "orange", "silver", "golden", "crimson", "scarlet", "indigo", "violet", "amber",
    "teal", "maroon", "copper", "bronze", "ivory",
    # naturaleza
    "thunder", "river", "ocean", "maple", "cedar", "willow", "phoenix", "cosmic", "comet", "nova",
    "blaze", "frost", "storm", "echo", "meadow", "canyon", "glacier", "summit", "harbor", "ember",
    "aurora", "eclipse", "tundra", "prairie", "lagoon", "breeze", "boulder", "cascade", "valley",
    "mountain", "forest", "desert", "island", "volcano", "geyser",
    # comida
    "coffee", "pepper", "mango", "cookie", "waffle", "pickle", "noodle", "biscuit", "honey", "mocha",
    "cocoa", "pretzel", "muffin", "donut", "taco", "peanut", "walnut", "cherry", "lemon", "ginger",
    "basil", "pesto", "cobbler",
    # positivo / misc
    "sunshine", "rocket", "ninja", "shadow", "rebel", "jasmine", "summer", "winter", "autumn",
    "maverick", "ranger", "pioneer", "voyager", "nomad", "drifter", "gizmo", "widget", "turbo",
    "neon", "pixel", "vortex", "quantum", "atlas", "titan", "orbit",
)
PW_SYMBOLS = "!@#$.-_*"   # sin ':' -> accounts.txt es email:password


def _gen_password() -> str:
    """Aleatoria pero plausible-humana: 1-2 palabras capitalizadas + número + símbolo opcional.
    Estilo RockYou, no char-soup. >=10 chars; siempre upper+lower+dígito -> pasa la política de
    Amazon (>=6) y no parece generada por máquina."""
    for _ in range(20):
        words = list(PW_WORDS)
        random.shuffle(words)
        n = secrets.choice((1, 2, 2))                   # casi siempre 2 palabras, a veces 1
        sep = secrets.choice(("", "", "-", "_", "."))   # mayoría CamelCase pegado
        core = sep.join(w.capitalize() for w in words[:n])
        num = secrets.choice((
            str(secrets.randbelow(90) + 10),            # 2 dígitos
            str(secrets.randbelow(900) + 100),          # 3 dígitos
            str(secrets.randbelow(46) + 1980),          # año 1980-2025
        ))
        pw = core + num
        if secrets.randbelow(3):                        # ~2/3 lleva símbolo al final
            pw += secrets.choice(PW_SYMBOLS)
        if len(pw) >= 10:                               # piso humano; 1 palabra corta -> reroll
            return pw
    return pw  # ponytail: inalcanzable en la práctica (cada try ~>90% pasa el piso)
...
def proxy_from_line(line: str) -> dict | None:
    """Una línea de proxies.txt -> dict proxy de Playwright. Dos formatos:
    - URL:    scheme://user:pass@host:port   (scheme http o https; https = proxy-over-TLS, NECESARIO
              con Decodo: su endpoint HTTP resetea el CONNECT de targets HTTPS, el HTTPS los tuneliza)
    - Decodo: host:port:user:pass            (split(':',3) -> el password puede contener ':')
    None si no trae al menos host:port."""
    line = line.strip()
    if "://" in line:
        u = urllib.parse.urlsplit(line)
        if not u.hostname or not u.port:
            return None
        proxy = {"server": f"{u.scheme or 'http'}://{u.hostname}:{u.port}"}
        if u.username:
            proxy["username"] = urllib.parse.unquote(u.username)
...
def _proxy_ip(proxy, timeout: float = 8.0):
    """Pre-vuelo: devuelve la IP pública del proxy (GET HTTP a api.ipify.org A TRAVÉS del proxy), o
    None si no conecta. Doble función:
    (1) Liveness — evita LANZAR el browser sobre un puerto Decodo muerto: un launch fallido deja vivo
        el loop asyncio de Camoufox y el siguiente intento muere con 'Sync API inside the asyncio loop'.
    (2) La IP se le pasa a Camoufox como geoip=<ip> para que NO haga su propia detección: la suya usa
        HTTPS y Decodo la RESETEA -> 'Failed to get IP address'.
    OJO: HTTP, NO HTTPS. El CONNECT https de urllib lo resetea Decodo aun en proxies VIVOS (fingerprint
    TLS no-browser); el GET http valida liveness y trae la IP. Barato. Sin proxy -> None (directo)."""
    if not proxy:
        return None
    server = proxy["server"]
    if "://" in server:  # urllib NO hace TLS-al-proxy -> el chequeo va por http-al-proxy (Decodo lo
        server = "http://" + server.split("://", 1)[1]  # acepta en el mismo puerto); trae la IP igual
    if proxy.get("username"):
        scheme, rest = server.split("://", 1)
        cred = f"{urllib.parse.quote(proxy['username'])}:{urllib.parse.quote(proxy.get('password', ''))}"
        server = f"{scheme}://{cred}@{rest}"
    opener = urllib.request.build_opener(
        urllib.request.ProxyHandler({"http": server, "https": server}))
    try:
        ip = opener.open("http://api.ipify.org/?format=text", timeout=timeout).read().decode().strip()
        return ip or None
    except Exception as e:
        print(f"🔌 proxy sin conexión ({e})")
        return None


def _ipqs_verdict(data: dict, max_fraud: int) -> bool:
    """¿La IP pasa el filtro de reputación IPQS? Bloquea SOLO por fraude real: recent_abuse o
    fraud_score >= max_fraud. NO mira los flags proxy/vpn —IPQS marca TODO proxy residencial y nos
    dejaría sin pool—. Respuesta sin score (success=false / dict vacío) -> deja pasar (no romper)."""
    if not data or data.get("success") is False:
        return True
    if data.get("recent_abuse"):
        return False
    return int(data.get("fraud_score", 0)) < max_fraud


def _ip_reputation_ok(ip) -> bool:
    """Pre-chequeo de reputación con IPQS (GET DIRECTO, no por el proxy) para descartar IPs malas
    ANTES de gastar warmup+signup. Sin IPQS_KEY -> no filtra (graceful). API caída/timeout -> pasa
    (no bloquear el flujo por un fallo de la API). Umbral por AMZ_IP_MAX_FRAUD (default 90)."""
    key = os.environ.get("IPQS_KEY")
    if not key or not ip:
        return True
    max_fraud = int(os.environ.get("AMZ_IP_MAX_FRAUD", "90"))
    try:
        url = f"https://ipqualityscore.com/api/json/ip/{key}/{urllib.parse.quote(ip)}?strictness=1"
        data = json.loads(urllib.request.urlopen(url, timeout=10).read())
    except Exception as e:
        print(f"⚠️  IPQS no respondió ({e}); dejo pasar")
        return True
    ok = _ipqs_verdict(data, max_fraud)
    print(f"{'✅' if ok else '🚫'} IP {ip} reputación {'ok' if ok else 'MALA'} "
          f"(fraud={data.get('fraud_score')}, recent_abuse={data.get('recent_abuse')})")
    return ok
...
def save_login(email: str, password: str, path) -> None:
    """Guarda email:password junto a las cookies para login manual posterior."""
    Path(path).write_text(f"{email}:{password}\n")
    print(f"🔑 login -> {path}")
...
def _select_country(page) -> bool:
    """Fuerza país = United States en el form de dirección. Necesario con proxy NO-US: el form
    default-ea al país de la IP y el alta US fallaría. Capas como _select_state: <select> nativo por
    value 'US'/label -> JS set value+change (oculto/sin CSS) -> dump+seguir. Sin selector de país
    (IP US, el form ya viene US) es no-op silencioso (False)."""
    ...
    if not present:
        return False
    for css in present:  # (1) <select> nativo visible
    ...
    for css in present:  # (2) JS set aunque esté oculto / sin CSS
        try:
            if page.locator(css).first.evaluate(js):
                return True
        except Exception:
            pass
    try:  # (3) dump para diagnóstico
        html = page.locator(present[0]).first.evaluate("el => el.outerHTML")
        dbg = COOKIES_DIR / "country_debug.html"
        dbg.write_text(html, encoding="utf-8")
        print(f"⚠️  país no seleccionable; HTML del control -> {dbg}")
    except Exception:
...
def _fill_and_submit_address(page, a: dict) -> None:
    ...
    # Con proxy NO-US el form default-ea al país de la IP -> forzar US ANTES de llenar (cambiar el
    # país puede re-renderizar estado/zip). Con IP US no hay selector de país -> no-op.
    if _select_country(page):
...
def _read_page(page, lo, hi):
    """Lectura humana: idle con micro-drift + a veces hover sobre un link y selección de un párrafo.
    Genera eventos de input REALES (mouseover + selección de texto) — la telemetría conductual que el
    anti-bot puntúa, no eventos sintéticos. Best-effort: cada gesto en su try/except con timeout corto
    (bounding_box acotado -> nunca cuelga la lectura)."""
    _idle(page, lo, hi)
    if random.random() < 0.5:  # hover-dwell sobre un link -> mouseover/mouseenter real
        try:
            links = page.locator("a:visible")
            n = links.count()
            if n:
                box = links.nth(random.randrange(min(n, 12))).bounding_box(timeout=1000)
                if box:
                    _human_move(page, box["x"] + box["width"] * random.uniform(0.2, 0.8),
                                box["y"] + box["height"] * random.uniform(0.2, 0.8))
                    _pause(page, 300, 1100)
        except Exception:
            pass
    if random.random() < 0.4:  # seleccionar un párrafo -> mouse down/move/up reales + getSelection
        try:
            paras = page.locator("p:visible")
            n = paras.count()
            if n:
                box = paras.nth(random.randrange(min(n, 8))).bounding_box(timeout=1000)
                if box and box["width"] > 40:
                    y = box["y"] + box["height"] / 2
                    _human_move(page, box["x"] + 4, y)
                    page.mouse.down()
                    _human_move(page, box["x"] + box["width"] * random.uniform(0.4, 0.9), y)
                    page.mouse.up()
                    _pause(page, 250, 800)
        except Exception:
            pass
...
def _follow_internal_link(page, prof):
    """Sigue UN link interno (mismo sitio) y lee la 2ª página -> navegación con profundidad, no
    rebote tras una sola vista. Best-effort; vuelve atrás al terminar. Solo modo full (cuesta bytes)."""
    try:
        host = urllib.parse.urlsplit(page.url).hostname or ""
        link = page.locator(f"a[href^='/']:visible, a[href*='{host}']:visible").first
        link.wait_for(state="visible", timeout=3000)
        _human_click(page, link)
        page.wait_for_load_state("domcontentloaded", timeout=12000)
        _read_page(page, *prof["read"])
...
def _enter_amazon(page):
    """Entra a Amazon como humano: pasa por DuckDuckGo (busca 'amazon', lee) y navega a Amazon con
    Referer del buscador -> referrer chain real. El clic directo al resultado de DDG es frágil sobre
    proxies degradados (los a.result__a vienen ocultos / son ads de Bing) -> se omite; el Referer es
    lo fiable y es lo que pesa. Best-effort: DDG cae -> igual entra con Referer; si cae -> goto pelado."""
    engine = random.choice(SEARCH_ENGINES)
    query = random.choice(("amazon", "amazon.com", "amazon online shopping"))
    try:
        page.goto(engine["url"].format(q=urllib.parse.quote_plus(query)),
                  wait_until="domcontentloaded")
        _read_page(page, 600, 1600)
    except Exception as e:
        print(f"ℹ️  buscador saltado ({e})")
    try:
        page.goto(HOME_URL, referer=engine["base"], wait_until="domcontentloaded")
        print(f"🔎 entrada con Referer de {engine['name']}")
    except Exception:
...
def _warmup_profile() -> dict:
    ...
    if mode == "off":
        return {"mode": "off"}
    if mode == "full":
        return {"mode": "full", "sites": (2, 4), "searches": (1, 3),
                "products": (1, 2), "read": (1500, 3500), "depth": 1}
    return {"mode": "light", "sites": (1, 2), "searches": (1, 1),  # default (incl. inválidos)
            "products": (0, 1), "read": (600, 1400)}


def _warmup_web(page, prof):
    """Antes de Amazon, curiosea sitios neutrales LIVIANOS (texto) para sembrar historial: un perfil
    con historial real se ve menos 'recién nacido'. Cantidad/pausas salen de `prof` (AMZ_WARMUP); en
    full sigue un link interno por sitio (profundidad). Conducta (read/wander/scroll) real con
    telemetría de input; bytes mínimos. Best-effort y acotado."""
    ...
    for url in random.sample(NEUTRAL_SITES, k=k):
        try:
            ...
            _human_scroll(page)
            if prof.get("depth"):
                _follow_internal_link(page, prof)
        except Exception as e:
...
def _warmup(page, prof):
    ...
    walled = False
    try:
        _enter_amazon(page)  # entra desde un buscador (referrer real) en vez de goto directo
        walled = _continue_shopping(page)  # muro anti-bot = IP de baja reputación (home degradada)
        _read_page(page, rlo, rhi)
        _wander(page)
        _human_scroll(page)
...
# El driver Firefox de Playwright CRASHEA (coreBundle.js: pageError.location.url) cuando una página
# lanza un error JS sin 'location' -> tumba el proceso entero. El warmup pasa por más sitios (DDG,
# neutrales) -> más chance de pisar esa mina. Escudo: en sitios NO-Amazon, marcar como 'manejado'
# todo error/rechazo no capturado -> la página no emite el uncaught que detona el driver. Se EXCLUYE
# amazon.com a propósito (su superficie de login queda intacta para anti-detección).
PAGEERROR_SHIELD = """
(() => {
  if (location.hostname && location.hostname.endsWith('amazon.com')) return;
...
def launch_browser(proxy, headless: bool, geoip_ip: str | None = None):
    ...
    return Camoufox(
        ...
        geoip=geoip_ip or bool(proxy),     # IP explícita pre-detectada por HTTP, o auto (bool) si no hay
        humanize=0.8,                      # curva de cursor humana nativa, ≤0.8s por movimiento
        os=["macos", "windows", "linux"],  # pool de OS para la huella
        # uBlock OFF: Camoufox lo trae por defecto y, en perfil EFÍMERO, re-baja TODO su set de
        # listas (easylist/ublockorigin/malware-filter/jsdelivr/mirror gitee) por el proxy en CADA
        # lanzamiento. Con --all = GB residenciales tirados. El saver ya corta media/trackers.
        exclude_addons=[DefaultAddons.UBO],
    )


BAD_PROXY = 3  # rc centinela interno: warmup detectó muro (IP degradada) -> main rota de proxy
MAX_PROXY_TRIES = 6  # intentos de IP por cuenta (pre-vuelo descarta puertos muertos; Decodo flakea)


def run_account(account: dict, proxy, headless: bool, skip_add: bool, hold: bool,
                bail_on_wall: bool = False, proxy_ip=None) -> int:
    ...
    with launch_browser(proxy, headless, proxy_ip) as browser:
        ...
        page.context.add_init_script(PAGEERROR_SHIELD)  # blinda el driver FF en sitios del warmup
        ...
        click_if_present(page, "text=Quedarse en Amazon.com", "text=Stay on Amazon.com", "text=Dismiss")
...
def run_signup(proxy, headless: bool, hold: bool, bail_on_wall: bool = False, proxy_ip=None) -> tuple[dict, int]:
    ...
    with launch_browser(proxy, headless, proxy_ip) as browser:
        ...
        if "/a/addresses" in page.url:
            ...
            save_login(email, password, COOKIES_DIR / f"{slug}.login.txt")
            added = None
            try:
                added = add_random_address(page)
            except Exception as e:
    ...
    if signup_n:
        ...
        for idx in range(signup_n):
            ...
            for attempt in range(MAX_PROXY_TRIES):
                ...
                proxy_ip = _proxy_ip(proxy)  # IP pública (HTTP) -> liveness + geoip explícito a Camoufox
                if proxy and not proxy_ip:  # puerto muerto/flaky -> rotar SIN lanzar browser
                    print(f"🔁 proxy muerto -> otra IP ({attempt + 2}/{MAX_PROXY_TRIES})")
                    continue
                if proxy_ip and not _ip_reputation_ok(proxy_ip):  # mala reputación -> rotar sin lanzar
                    print(f"🔁 IP {proxy_ip} mala reputación -> otra IP ({attempt + 2}/{MAX_PROXY_TRIES})")
                    continue
                try:  # IP degradada (muro/intent atascado) -> BAD_PROXY -> rotar; último procede igual
                    new_acct, rc = run_signup(proxy, headless, hold, bail_on_wall=not last, proxy_ip=proxy_ip)
                except Exception as e:
            ...
            signup_delay = int(os.environ.get("AMZ_SIGNUP_DELAY", "180"))
            if idx < signup_n - 1 and signup_delay:  # throttle anti-velocity entre cuentas (no rafaguear)
                wait = signup_delay + random.randint(0, max(1, signup_delay // 3))
                print(f"⏳ throttle anti-velocity: {wait}s antes de la próxima cuenta...")
                time.sleep(wait)
        return worst
    ...
    for account in targets:
        ...
        for attempt in range(MAX_PROXY_TRIES):
            ...
            try:  # IP degradada (muro) -> BAD_PROXY -> rotar; en el último intento procede igual
                rc = run_account(account, proxy, headless, skip_add, hold, bail_on_wall=not last, proxy_ip=proxy_ip)
            except Exception as e:
...
def _selftest() -> None:
    ...
    # contraseña plausible-humana: única en lote, >=10 chars, upper+lower+dígito, basada en
    # palabras (no char-soup), sin ':' (no rompe el parser email:password de accounts.txt).
    pws = [_gen_password() for _ in range(200)]
    assert len(set(pws)) == len(pws), "contraseñas repetidas"
    for pw in pws:
        assert len(pw) >= 8, pw
        assert any(c.isupper() for c in pw) and any(c.islower() for c in pw) and any(c.isdigit() for c in pw), pw
        assert any(w in pw.lower() for w in PW_WORDS), pw
        assert ":" not in pw, pw
    ...
    assert SEARCH_ENGINES, "SEARCH_ENGINES vacío -> _enter_amazon sin buscador"
    ...
    assert "onerror" in PAGEERROR_SHIELD and "amazon.com" in PAGEERROR_SHIELD
    ...
    assert all(callable(f) for f in (_vp, _prime_viewport, _wander, _human_scroll, _human_click, _human_type,
                                     _human_move, _idle, _warmup_search, _read_page, _enter_amazon,
                                     _follow_internal_link, _install_saver_routes,
                                     launch_browser, _select_state, _fill_and_submit_address,
                                     _warmup_profile, _await_address_ok, _address_listed,
                                     _continue_shopping, _dismiss_not_found, _cvf_error, _proxy_ip,
                                     _ip_reputation_ok, _ipqs_verdict, _select_country))
    ...
    light_prof = _warmup_profile()
    assert light_prof["mode"] == "light", "default debe ser light"
    assert light_prof["sites"][0] >= 1, "light siempre siembra >=1 sitio (varios sitios)"
    os.environ["AMZ_WARMUP"] = "off"
    assert _warmup_profile()["mode"] == "off"
    os.environ["AMZ_WARMUP"] = "FULL"
    full_prof = _warmup_profile()
    assert full_prof["mode"] == "full"
    assert full_prof["sites"][0] >= 2 and full_prof.get("depth"), full_prof
    ...
    # formato URL scheme://user:pass@host:port (https = proxy-over-TLS para targets HTTPS de Decodo)
    up = proxy_from_line("https://user-x-sessionduration-15:r10pSBdhtc4swMJ3o+@us.decodo.com:10001")
    assert up == {"server": "https://us.decodo.com:10001",
                  "username": "user-x-sessionduration-15", "password": "r10pSBdhtc4swMJ3o+"}, up
    assert proxy_from_line("http://u:p@h:8080")["server"] == "http://h:8080", proxy_from_line("http://u:p@h:8080")
    ...
    # IPQS: bloquea por fraude real (recent_abuse / fraud_score alto), NUNCA por flag proxy/vpn.
    assert _ipqs_verdict({"fraud_score": 10}, 90) is True
    assert _ipqs_verdict({"fraud_score": 95}, 90) is False
    assert _ipqs_verdict({"fraud_score": 0, "recent_abuse": True}, 90) is False
    assert _ipqs_verdict({"proxy": True, "vpn": True, "fraud_score": 20}, 90) is True  # proxy != bloqueo
    assert _ipqs_verdict({}, 90) is True                                  # sin datos -> pasa
    assert _ipqs_verdict({"success": False, "fraud_score": 100}, 90) is True  # error API -> pasa
    print("selftest OK")
