...
**Approach:** For cookie-mode (Amazon) replies, collapse an Approved verdict to exactly `⌿ Status: Approved ✅` and a Declined verdict to exactly `⌿ Status: Declined ❌`; any other reply (cookie errors, format help, plain edits) passes through unchanged. Verdict classification is delegated to the owner-locked `parse_amazon_verdict` (the same binding token-exact rule the capture engine stores by) so the displayed status can never disagree with the engine — replacing the transform's own weaker `startswith` parser. The transform is gated on the session `cookie_mode` flag (`capture_session.cookie_mode`, threaded to all four display surfaces) — NOT the gate name. This is load-bearing: the prior branded transform keyed off `"amz" in gate_name.lower()`, which the real gate name does not contain, so it was a silent no-op in production (the client saw raw bot text). Display-only: stored DB text is never mutated; the admin cross-tenant view stays raw.
