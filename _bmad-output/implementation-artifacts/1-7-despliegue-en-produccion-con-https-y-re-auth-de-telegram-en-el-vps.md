...
- [x] **CERRADO 2026-06-12:** re-auth ejecutado en el VPS — sesión autorizada como @maxff9778, `-rw------- cc cc /var/lib/cc/anon.session`, `TELEGRAM_TARGET=@Zephyrrchkbot`, cc-core reiniciado, health OK. [Review][Decision] AC4 unmet — `anon.session` absent in production. **RESOLVED (2026-06-11): deferred to owner** — re-auth is interactive (phone→code→2FA), only Richard can run it. REMAINING ACTION (Richard, on the VPS): set `TELEGRAM_API_ID`/`TELEGRAM_API_HASH` in `/srv/cc/backend/.env`, then `cd /srv/cc/backend && sudo .venv/bin/python -m scripts.telegram_auth`; verify `-rw------- cc cc /var/lib/cc/anon.session`. Blocks story → done.
- [x] **CERRADO 2026-06-12:** browser pass del owner — login, navegación e intento de lote real sobre HTTPS; 401 con error inline verificado a nivel curl en el deploy original. [Review][Decision] AC5 smoke test was curl-only — runbook step 11 requires browser-level checks. **RESOLVED (2026-06-11): record softened** to "curl-level, browser pass PENDING" (needs the owner's browser + password). REMAINING ACTION (Richard, 1 min): open `https://cc.lohari.com.mx`, login → home, check padlock, wrong password → inline Spanish error.
